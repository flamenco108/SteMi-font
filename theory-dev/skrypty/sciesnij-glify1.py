...
def find_glyphs_in_range(font, pua_start, pua_end, use_encoding_line):
    ...
    for glyph in font.glyphs():
        code_point = glyph.encoding if use_encoding_line else glyph.unicode
        if pua_start <= code_point <= pua_end:
...
def move_glyphs_within_range(font, pua_start, pua_end, use_encoding_line):
    ...
    for glyph in glyphs_in_range:
        original_code_point = glyph.encoding if use_encoding_line else glyph.unicode
...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        print("Usage: python sciesnij-glify.py <input_sfd> <output_sfd> <pua_start> <pua_end> <-n or -u>")
        sys.exit(1)
